...
Future<UsersHistoryResponse> getUserHistory(
  JikanClient client,
  String username, {
  String? type,
}) async {
  try {
    ...
    final encodedQueryParams = Uri(queryParameters: queryParams).query;
    final response = await client.httpClient.get(Uri.parse('${client.jikanV4BaseUrl}/users/$username/history?$encodedQueryParams'));
    if (response.statusCode == 200) {
      final jsonData = jsonDecode(response.body);
      return UsersHistoryResponse.fromJson(jsonData as Map<String, dynamic>);
    } else {
      throw JikanException(response.body);
    }
  } catch (e, trace) {
    if (e is! JikanException) {
      print('$getUserHistory: $trace');
    }
    rethrow;
  }
}
...
Future<UsersReviewsResponse> getUserReviews(
  JikanClient client,
  String username, {
  int? page,
}) async {
  try {
    ...
    final encodedQueryParams = Uri(queryParameters: queryParams).query;
    final response = await client.httpClient.get(Uri.parse('${client.jikanV4BaseUrl}/users/$username/reviews?$encodedQueryParams'));
    if (response.statusCode == 200) {
      final jsonData = jsonDecode(response.body);
      return UsersReviewsResponse.fromJson(jsonData as Map<String, dynamic>);
    } else {
      throw JikanException(response.body);
    }
  } catch (e, trace) {
    if (e is! JikanException) {
      print('$getUserReviews: $trace');
    }
    rethrow;
  }
}

Future<UsersRecommendationsResponse> getUserRecommendations(
  JikanClient client,
  String username, {
  int? page,
}) async {
  try {
    ...
    final encodedQueryParams = Uri(queryParameters: queryParams).query;
    final response = await client.httpClient.get(Uri.parse('${client.jikanV4BaseUrl}/users/$username/recommendations?$encodedQueryParams'));
    if (response.statusCode == 200) {
      final jsonData = jsonDecode(response.body);
      return UsersRecommendationsResponse.fromJson(jsonData as Map<String, dynamic>);
    } else {
      throw JikanException(response.body);
    }
  } catch (e, trace) {
    if (e is! JikanException) {
      print('$getUserRecommendations: $trace');
    }
    rethrow;
  }
}
...
Future<UsersExternalResponse> getUserExternal(
  JikanClient client,
  String username,
) async {
  try {
    final response = await client.httpClient.get(Uri.parse('${client.jikanV4BaseUrl}/users/$username/external'));
    if (response.statusCode == 200) {
      final jsonData = jsonDecode(response.body);
      return UsersExternalResponse.fromJson(jsonData as Map<String, dynamic>);
    } else {
      throw JikanException(response.body);
    }
  } catch (e, trace) {
    if (e is! JikanException) {
      print('$getUserExternal: $trace');
    }
    rethrow;
  }
}
