...
Future<ClubsRelationsResponse> getClubRelations(
  JikanClient client,
  int id,
) async {
  try {
    final response = await client.httpClient.get(Uri.parse('${client.jikanV4BaseUrl}/clubs/$id/relations'));
    if (response.statusCode == 200) {
      final jsonData = jsonDecode(response.body);
      return ClubsRelationsResponse.fromJson(jsonData as Map<String, dynamic>);
    } else {
      throw JikanException(response.body);
    }
  } catch (e, trace) {
    if (e is! JikanException) {
      print('$getClubRelations: $trace');
    }
    rethrow;
  }
}
