...
import 'package:jikan_moe/src/anime/anime_staff.dart';

import '../queue.dart';

void main() {
  group('JikanClient getAnimeStaff Tests', () {
    ...
    test('should handle $testCount specific anime IDs with queue-based rate limiting', () async {
      ...
      for (final id in testIds) {
        try {
          final result = await queue.add(() => client.getAnimeStaff(id));

          // Test: Should return List<AnimeStaff> type
          expect(result, isA<List<AnimeStaff>>(), reason: 'ID $id should return List<AnimeStaff>');

          print('✓ ID $id: Successfully parsed ${result.length} staff members');
          processedCount++;
